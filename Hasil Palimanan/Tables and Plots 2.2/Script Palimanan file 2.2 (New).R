...
setwd("D:/Kerjaan/Research Consultant/Project Cleaning Data mas Catur/Palimanan file 2.2")
folder_path <- "D:/Kerjaan/Research Consultant/Project Cleaning Data mas Catur/Palimanan file 2.2"
...
for (mdb_file in mdb_files) {
  ...
  berat_long <- tabel_ringkasan %>%
    select(Jumlah_Gandar, Berat_Rata_Rata, Berat_Minimal, Berat_Maksimal) %>%
  ...
  berat_plot <- ggplot(berat_long, aes(x = factor(Jumlah_Gandar), y = Berat, fill = Tipe)) +
    ...
    labs(title = paste("Berat Gandar (Axles)", file_name),
         x = "Jumlah Gandar Kendaraan", y = "Berat Gandar (ton)", fill = "Tipe Berat") +
  ...
  kecepatan_long <- tabel_ringkasan %>%
    select(Jumlah_Gandar, Kecepatan_Rata_Rata, Kecepatan_Minimal, Kecepatan_Maksimal) %>%
...
# Simpan workbook
saveWorkbook(wb, file = "Summary File Palimanan file 2.2.xlsx", overwrite = TRUE)
